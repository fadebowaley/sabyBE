  ...
  try {
    ...
    for (let i = 0; i < folderNames.length; i++) {
      try {
        ...
        console.log(
          `✅ Created folder: ${folderResponse.data.name} (ID: ${folderResponse.data.id})`
        );
      } catch (error) {
        console.log(
          `⚠️  Folder ${folderNames[i]} creation issue:`,
    ...
    for (let i = 0; i < 5; i++) {
      ...
      try {
        ...
        console.log(
          `✅ Uploaded: ${uploadResponse.data.originalName} (${uploadResponse.data.fileSize} bytes)`
        );
    ...
    if (createdFiles.length > 0) {
      ...
      if (createdFiles[0]) {
        try {
          await axios.patch(
            `${BASE_URL}/storage/${createdFiles[0].id}`,
            {
              isPublic: true,
            },
            {
      ...
      if (createdFiles[1]) {
        try {
          const shareResponse = await axios.post(
            `${BASE_URL}/storage/${createdFiles[1].id}/share`,
            {
              allowDownload: true,
              allowPreview: true,
      ...
      if (createdFiles[2]) {
        try {
          const shareResponse = await axios.post(
            `${BASE_URL}/storage/${createdFiles[2].id}/share`,
            {
              allowDownload: true,
              password: 'secure123',
            },
      ...
      if (createdFiles[3]) {
        try {
          const copyResponse = await axios.post(
            `${BASE_URL}/storage/${createdFiles[3].id}/copy`,
            {
      ...
      if (createdFiles[4] && createdFolders.length > 1) {
        try {
          await axios.post(
            `${BASE_URL}/storage/${createdFiles[4].id}/move`,
            {
    ...
    if (createdFolders.length > 0) {
      ...
      // Share first folder
      try {
        const folderShareResponse = await axios.post(
          `${BASE_URL}/storage/folders/${createdFolders[0].id}/share`,
          {
            allowUpload: true,
      ...
      try {
        const contentsResponse = await axios.get(
          `${BASE_URL}/storage/folders/${createdFolders[0].id}/contents`,
          {
        ...
        console.log(
          `✅ Folder contents retrieved: ${contentsResponse.data.files.length} files, ${contentsResponse.data.subfolders.length} subfolders`
        );
      } catch (error) {
        console.log(
          '⚠️  Get contents failed:',
          error.response?.data?.message || error.message
        );
      }
      console.log('');
    ...
    try {
      const searchResponse = await axios.get(
        `${BASE_URL}/storage/search?q=document`,
        {
          headers: { Authorization: `Bearer ${authToken}` },
        }
      );
      ...
      console.log(
        `✅ Search results: ${searchResponse.data.length} files found`
      );
      searchResponse.data.forEach((file, index) => {
        console.log(
          `   ${index + 1}. ${file.originalName} (${file.fileSize} bytes)`
        );
      });
    } catch (error) {
      console.log(
    ...
    try {
      ...
      console.log(
        `   📊 Average File Size: ${Math.round(
          statsResponse.data.avgFileSize
        )} bytes`
      );
      console.log(
        `   🗄️  Storage Used: ${statsResponse.data.quota.usedStorage} bytes`
      );
      console.log(
        `   📈 Usage Percentage: ${statsResponse.data.usagePercentage.toFixed(
          2
        )}%`
      );
    } catch (error) {
