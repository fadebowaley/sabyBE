...
exports.handlePhoneContact = async (bot, msg, session) => {
  ...
  try {
    logger.info(`🔐 Processing phone authentication for chat ${chatId} (${phoneNumber})`);

    // First, get the user by phone number to find their tenant
    const user = await userService.getUserByPhone(phoneNumber);

    if (!user) {
      logger.warn(`❌ Phone authentication failed for ${phoneNumber}: User not found`);
    ...
    const userValidation = await telegramValidationService.validateUserByPhone(phoneNumber, tenantId);

    if (!userValidation.valid) {
      logger.warn(`❌ Phone authentication failed for ${phoneNumber}: ${userValidation.error}`);
      await telegramNotificationService.sendAuthenticationFailure(chatId, userValidation.error);
      return;
    }
    ...
    session.metadata.userName =
      validatedUser.name || `${validatedUser.firstname || ''} ${validatedUser.lastname || ''}`.trim();
    await session.save();

    logger.info(`✅ Phone authentication successful for ${phoneNumber} (User ID: ${validatedUser._id})`);

    // Get available projects for the user
    logger.info(`🔍 Getting available projects for tenant: ${validatedUser.tenantId}`);
    const availableProjectsResult = await getAvailableProjects(validatedUser.tenantId);
    logger.info(`🔍 Available projects result:`, availableProjectsResult);

    // Handle paginated result
    const availableProjects =
      availableProjectsResult && availableProjectsResult.results ? availableProjectsResult.results : availableProjectsResult;
    ...
    await telegramNotificationService.sendAuthenticationSuccess(chatId, validatedUser, availableProjects);

    logger.info(`✅ Authentication success message sent to chat ${chatId}`);
  } catch (error) {
    logger.error(`❌ Error processing phone authentication for chat ${chatId}:`, error.message);
    logger.error(`❌ Full error details:`, error);
    await telegramNotificationService.sendAuthenticationFailure(chatId, 'Authentication failed. Please try again.');
  }
};
...
exports.handleProjectSelection = async (bot, msg, session) => {
  ...
  try {
    ...
    const availableProjectsResult = await getAvailableProjects(session.tenantId);
    ...
    const selectedProject = availableProjects.find(
      (project) => project.configuration?.projectName === selectedProjectName || project.projectId === selectedProjectName
    );
    ...
    const projectValidation = await telegramValidationService.validateProjectForm(selectedProject.projectId, {
      tenantId: session.tenantId,
    });

    if (!projectValidation.valid) {
      logger.warn(`❌ Project validation failed for ${selectedProject.projectId}: ${projectValidation.error}`);
      await telegramNotificationService.sendAuthenticationFailure(chatId, projectValidation.error);
      return;
    }
...
async function getAvailableProjects(tenantId) {
  try {
    ...
    logger.info(`🔍 projectFormService.getProjectFormsByTenant result:`, projectsResult);

    // Handle paginated result
    const projects = projectsResult && projectsResult.results ? projectsResult.results : projectsResult;
    logger.info(`🔍 Projects type: ${typeof projects}, length: ${projects ? projects.length : 'undefined'}`);
...
exports.handleManualPhone = async (bot, msg, session) => {
  ...
  try {
    logger.info(`📱 Processing manual phone input for chat ${chatId}: ${phoneNumber}`);
    ...
    if (!phoneRegex.test(phoneNumber.replace(/[\s\-\(\)]/g, ''))) {
      await telegramNotificationService.sendValidationError(chatId, {
        field: 'Phone Number',
        error: 'Invalid phone number format. Please use the share button or enter a valid number.',
      });
      return;
    }
    ...
    logger.error(`❌ Error processing manual phone input for chat ${chatId}:`, error.message);
...
async function sendAuthenticationSuccess(bot, chatId, user, projects) {
  try {
    ...
    projects.forEach((project, index) => {
      console.log(`🔍 Project ${index + 1}: ${project.configuration?.projectName || project.projectId}`);
    });
    ...
    const keyboard = {
      inline_keyboard: [
        ...
        ...projects.map((project, index) => [
          {
            text: `${getProjectEmoji(index)} ${project.configuration?.projectName || project.projectId}`,
    ...
    console.error(`❌ Error sending authentication success to chat ${chatId}:`, error.message);
    throw error;
  }
}
