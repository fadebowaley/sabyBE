 ...
 * Handles validation of Telegram submissions including:
...
class TelegramValidationService {
   ...
   * @param {string} phoneNumber - Phone number from Telegram contact
  ...
  async validateUserByPhone(phoneNumber, tenantId) {
    try {
      logger.info(`🔍 Validating Telegram user by phone: ${phoneNumber} for tenant: ${tenantId}`);

      // Find user by phone number
      const user = await userService.getUserByPhone(phoneNumber);
      console.log(`[telegramValidationService] getUserByPhone(${phoneNumber}) result:`, user);
      if (!user) {
        logger.warn(`❌ Telegram user not found: ${phoneNumber}`);
        return { valid: false, error: 'User is not registered', code: 'USER_NOT_FOUND' };
      }

      // Check if user belongs to the tenant
      if (user.tenantId !== tenantId) {
        logger.warn(`❌ Telegram user ${phoneNumber} does not belong to tenant ${tenantId}`);
        return { valid: false, error: 'User does not belong to this tenant', code: 'TENANT_MISMATCH' };
      }

      // Check if user is active (not soft deleted)
      if (user.deletedAt) {
        logger.warn(`❌ Telegram user ${phoneNumber} is deleted`);
        return { valid: false, error: 'User account is inactive', code: 'USER_DELETED' };
      }

      logger.info(`✅ Telegram user validated: ${phoneNumber} (User ID: ${user._id})`);
      return { valid: true, user };
    } catch (error) {
      logger.error(`❌ Error validating Telegram user ${phoneNumber}:`, error.message);
  ...
  async validateProjectForm(projectId, user) {
    try {
      logger.info(`🔍 Validating project form: ${projectId} for Telegram user: ${user.phone}`);
  ...
  /**
   * Validate form fields for Telegram submission
   * @param {Object} answers - Answers from Telegram user
   * @param {Object} projectForm - Project form object
   * @returns {Promise<Object>} Validation result
   */
  async validateFormFields(answers, projectForm) {
    try {
      logger.info(`🔍 Validating form fields for Telegram project: ${projectForm.projectId}`);
      ...
      if (validationResult.valid) {
        logger.info(`✅ Form fields validated successfully for Telegram project: ${projectForm.projectId}`);
      } else {
        logger.warn(`⚠️ Form validation failed for Telegram project: ${projectForm.projectId}`, {
  ...
  validateFile(value, properties) {
    // For Telegram submissions, files come as document/photo messages
  ...
  /**
   * Send validation failure notification to user via Telegram
  ...
  /**
   * Comprehensive validation of Telegram submission
   * @param {Object} sessionData - Session data with user info
   * @param {Object} answers - User answers
   * @param {string} projectId - Project ID
   * @returns {Promise<Object>} Complete validation result
   */
  async validateTelegramSubmission(sessionData, answers, projectId) {
    const phoneNumber = sessionData.phoneNumber;
    const tenantId = sessionData.tenantId;

    logger.info(`🔍 Starting comprehensive validation for Telegram submission from ${phoneNumber} to project ${projectId}`);
    ...
    try {
      ...
      logger.info(
        `✅ Telegram submission validation successful for ${phoneNumber} to project ${projectId} (tenant: ${tenantId})`
      );
      return validationResult;
    } catch (error) {
      logger.error(`❌ Error during Telegram submission validation:`, error.message);
      validationResult.errors.push({
...
module.exports = new TelegramValidationService();
