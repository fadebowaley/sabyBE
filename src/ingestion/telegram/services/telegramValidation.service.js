...
class TelegramValidationService {
  ...
  async validateUserByPhone(phoneNumber, tenantId) {
    try {
      logger.info(
        `🔍 Validating Telegram user by phone: ${phoneNumber} for tenant: ${tenantId}`
      );

      // Find user by phone number
      const user = await userService.getUserByPhone(phoneNumber);
      console.log(
        `[telegramValidationService] getUserByPhone(${phoneNumber}) result:`,
        user
      );
      if (!user) {
      ...
      if (user.tenantId !== tenantId) {
        logger.warn(
          `❌ Telegram user ${phoneNumber} does not belong to tenant ${tenantId}`
        );
        return {
          valid: false,
      ...
      logger.info(
        `✅ Telegram user validated: ${phoneNumber} (User ID: ${user._id})`
      );
      return { valid: true, user };
    } catch (error) {
      logger.error(
        `❌ Error validating Telegram user ${phoneNumber}:`,
        error.message
      );
      return {
        valid: false,
  ...
  async validateProjectForm(projectId, user) {
    try {
      logger.info(
        `🔍 Validating project form: ${projectId} for Telegram user: ${user.phone}`
      );
  ...
  async validateFormFields(answers, projectForm) {
    try {
      logger.info(
        `🔍 Validating form fields for Telegram project: ${projectForm.projectId}`
      );
      ...
      if (validationResult.valid) {
        logger.info(
          `✅ Form fields validated successfully for Telegram project: ${projectForm.projectId}`
        );
      } else {
        logger.warn(
          `⚠️ Form validation failed for Telegram project: ${projectForm.projectId}`,
          {
  ...
  async validateTelegramSubmission(sessionData, answers, projectId) {
    ...
    logger.info(
      `🔍 Starting comprehensive validation for Telegram submission from ${phoneNumber} to project ${projectId}`
    );
    ...
    try {
      ...
      logger.error(
        `❌ Error during Telegram submission validation:`,
        error.message
      );
      validationResult.errors.push({
