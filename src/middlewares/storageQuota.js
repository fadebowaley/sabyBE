      ...
      if (operation === 'upload') {
        ...
        if (usedStorage + uploadSize > totalQuota) {
          ...
          throw new ApiError(
            ...
            `Storage quota exceeded. Available space: ${Math.max(
              0,
              remainingSpace
            )} bytes`
          );
        ...
        const maxFileSize =
          settings.userLimits?.maxFileSize || 100 * 1024 * 1024; // 100MB
 ...
 */
const checkFileTypes = () => async (req, res, next) => {
  try {
    ...
    const { allowedFileTypes = ['*'], blockedFileTypes = [] } =
      settings.userLimits;
    ...
    const checkFileType = (file) => {
      ...
      if (
        blockedFileTypes.includes(fileExtension) ||
        blockedFileTypes.includes(mimeType)
      ) {
 ...
 */
const checkQuotaWarning = () => async (req, res, next) => {
  try {
    const settings = req.storageSettings;
    ...
    if (usagePercentage >= quotaWarningThreshold) {
      ...
      console.log(
        `Quota warning: ${usagePercentage.toFixed(1)}% used for tenant ${
          req.user.tenantId
        }`
      );
    }
